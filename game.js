...
function renderDiceThemeBtn(btn) {
  ...
  btn.setAttribute(
    "aria-label",
    "Dice style: " + set.charAt(0).toUpperCase() + set.slice(1),
  );
}
...
function closeDiceThemePicker() {
  ...
  document
    .querySelectorAll(".dice-theme-btn.active")
...
function showDiceThemePicker(btn) {
  ...
  [
    ...
    opt.className =
      "dice-theme-option" + (key === currentSet ? " selected" : "");
  ...
  setTimeout(
    () =>
      document.addEventListener("click", closeDiceThemePicker, { once: true }),
    0,
  );
}
...
// SCREEN NAVIGATION
// ============================================================
const SCREENS = ["setup-screen", "game-screen", "gameover-screen"];
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("new-game-btn").addEventListener("click", () => {
    if (confirm("Abandon this game and start over?")) showSetupScreen();
  });
  [
    "opt-entry",
    "opt-clear-flash",
...
function removePlayer(btn) {
  ...
  document.getElementById("add-player-btn").disabled = false;
  // Re-number
  document.querySelectorAll(".player-num").forEach((el, i) => {
    el.textContent = i + 1;
  });
}
...
function onStartGame() {
  ...
  const diceSets = Array.from(document.querySelectorAll(".dice-theme-btn")).map(
...
function saveGameState() {
  ...
  try {
    ...
    if (snapshot.phase === "rolling") {
      ...
      snapshot.dice.forEach((d) => {
        if (d.state === "rolling") d.state = "unrolled";
      });
...
function restoreGameState() {
  try {
    ...
    if (!snapshot || !snapshot.players || snapshot.phase === "gameover")
      return false;
    G = { ...snapshot, lastLicksDone: new Set(snapshot.lastLicksDone) };
    refreshDiceFaces(
      (G.players[G.currentPlayerIndex] || {}).diceSet || "cosmic",
    );
    showScreen("game-screen");
    renderScoreBoard();
    renderCurrentPlayer();
    ...
    if (G.phase === "pact-freight-choice" && G.evalResult)
      showFreightPactChoice(G.evalResult);
    ...
    if (msg) {
      ...
      setMessage(
        `${G.players[G.currentPlayerIndex].name}'s turn — game restored.`,
        "",
      );
    }
    return true;
...
function createGame(playerNames, opts = {}, diceSets = []) {
  return {
    players: playerNames.map((name, i) => ({
      ...
      score: 0,
      eliminated: false,
      suicidePactToken: false,
      diceSet: diceSets[i] || "cosmic",
    })),
    opts: {
  ...
  if (n === 5) {
    ...
    if (unique.size === 1) {
      ...
      if (nonNull.length + (hasSun ? 1 : 0) === 5) {
        if (fv === 6)
          return { type: "freight", sub: "win", value: fv, score: 600 };
        if (fv === 10)
          return { type: "freight", sub: "supernova", value: fv, score: 0 };
...
function calcCurrentRollScore() {
  ...
  const allHeld =
    ...
    flashIndices.every(
      (li) =>
        G.dice[gIdxs[li]].state === "flash" ||
        G.dice[gIdxs[li]].state === "selected",
    );
...
function refreshTurnScore() {
  document.getElementById("turn-score-val").textContent =
...
function startTurn() {
  ...
  const msg = G.lastLicksActive
    ...
    : `${player.name}'s turn!`;
  const sub =
    G.opts.entryRequired && player.score === 0
...
function onRoll() {
  ...
  if (rollAll) {
    ...
    rollIndices = G.dice
      .map((d, i) => (d.state !== "committed" ? i : null))
      .filter((i) => i !== null);
  }
...
function applyRollRules(er, opts, rollIndices, dice) {
  ...
  const allUsed = afterStates.every(
    (s) => s === "committed" || s === "selected" || s === "flash",
  );
...
function resolveRoll(rollIndices, prerolledValues) {
  ...
  rollIndices.forEach((gi, li) => {
    G.dice[gi].value = prerolledValues
      ? prerolledValues[li]
      : rollDie(G.dice[gi].isBlack);
  ...
  if (er.flashValue !== null) {
    const flashSub = G.opts.flashOptional
      ...
      : G.mustRollAll
        ? "All dice used — must roll all 5 to clear!"
        : "Must roll remaining dice to clear.";
    setMessage(
      `Flash of ${er.flashValue}s! (+${er.flashValue * 10} pts)`,
      flashSub,
    );
    ...
    if (G.opts.allFiveRequired) {
      setMessage(
        "All dice scored! Must roll all 5 again.",
        "Banking disabled until you roll.",
      );
    } else {
      setMessage(
        "All dice scored! Roll all 5 to chain, or bank.",
        "Roll All 5 to keep building your score.",
      );
    }
    ...
    setMessage(
      "Scoring dice selected",
      hasToggleable
        ? "Tap 5s or 10s to toggle · Gold = locked in · Cyan = flash"
        : "Roll remaining or Bank.",
    );
  }
  saveGameState();
}
...
function handleFreight(er) {
  ...
  if (er.sub === "supernova") {
    ...
    setMessage(
      "💥 SUPERNOVA! Five 10s!",
      `${G.players[G.currentPlayerIndex].name} is eliminated!`,
    );
...
function invokeSuicidePact() {
  const flashValue =
    WHITE_FACES[Math.floor(Math.random() * WHITE_FACES.length)];
...
function handleWimpout() {
  ...
  setMessage(
    "💨 WIMPOUT!",
    lost > 0
      ? `Lost ${lost} turn pts — pass the dice.`
      : "No points scored — pass the dice.",
  );
  setMessageClass("msg-wimpout");
...
function onBank() {
  ...
  if (G.opts.entryRequired && player.score === 0 && total < ENTRY_SCORE) {
    setMessage(
      `Need ${ENTRY_SCORE} pts to get on the board!`,
      `You have ${total} — keep rolling!`,
    );
    return;
  }
  if (rollScore === 0) {
...
function bankAndFinish(instantWin) {
  ...
  if (hitTarget && !G.lastLicksActive) {
    ...
    setMessage(
      `${player.name} reaches ${player.score} pts!`,
      "⚡ LAST LICKS — everyone else gets one final turn!",
    );
    ...
    setMessage(
      `${player.name} scores ${player.score} pts!`,
      "High score wins after all last licks!",
    );
  } else {
...
function checkGameOver() {
  ...
  if (G.lastLicksActive) {
    ...
    const needLick = G.players.filter(
      (p, i) => i !== initiator && !p.eliminated,
    );
    const allDone = needLick.every((p) =>
      G.lastLicksDone.has(G.players.indexOf(p)),
    );
    if (allDone) {
      showGameOver();
      return true;
  ...
  if (G.phase === "choosing") {
    const hasToggle = G.lastRollIndices.some((gi) => {
      ...
      return (
        d.state === "selected" &&
        G.evalResult &&
        !G.evalResult.flashIndices.includes(li)
      );
    });
    hint.textContent = hasToggle
      ? "Tap gold dice to un-select · Cyan = flash (locked) · Gold = selected"
      : "";
  } else {
    hint.textContent = "";
  }
...
function updateButtons() {
  ...
  if (G.phase === "choosing") {
    ...
    const forceRollAll =
      G.mustRollAll || (G.opts.allFiveRequired && allDiceUsed);
    ...
    const player = G.players[G.currentPlayerIndex];
    const entryOk =
      !G.opts.entryRequired || player.score > 0 || total >= ENTRY_SCORE;
    const canBank =
      !G.mustClearFlash && !forceRollAll && entryOk && rollScore > 0;
